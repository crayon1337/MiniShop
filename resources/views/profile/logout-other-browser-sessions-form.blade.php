<x-jet-action-section>
    <x-slot name="title">
        {{ __('misc.browserSessions') }}
    </x-slot>

    <x-slot name="description">
        {{ __('misc.browserSessionsDesc') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('misc.browserSessionsText') }}
        </div>
                        ...
                        <div class="ml-3">
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @else
                                        {{ __('misc.lastActive') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('misc.logOutBrowserText') }}

                <div class="mt-4" x-data="{}" x-on:confirming-logout-other-browser-sessions.window="setTimeout(() => $refs.password.focus(), 250)">
                    <x-jet-input type="password" class="mt-1 block w-3/4" placeholder="{{ __('misc.password') }}"
                                x-ref="password"
